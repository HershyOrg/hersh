...
func TestWatchFlowBasic(t *testing.T) {
	...
	getChannelFunc := func(ctx context.Context) (<-chan shared.FlowValue, error) {
		testChan := make(chan shared.FlowValue, 10)

		go func() {
			defer close(testChan)

			// Send initial value
			testChan <- shared.FlowValue{V: 0, E: nil}
			...
			for i := 1; i <= 5; i++ {
				...
				case <-time.After(200 * time.Millisecond):
					...
					testChan <- shared.FlowValue{V: i, E: nil}
				}
			}
		}()
	...
	watcher.Manage(func(msg *hersh.Message, ctx hersh.HershContext) error {
		...
		// WatchFlow
		val := hersh.WatchFlow(getChannelFunc, "test_value", ctx)
		if val.Value != nil {
